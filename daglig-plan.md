# Espen sin dag-til-dag sjekkliste – R2 eksamen 15. mai 2026

> **Start:** 15. mars 2026 | **Eksamen:** 15. mai 2026
> Sett ✅ når ferdig. Hopp over og noter hvis noe ikke ble gjort – ingen drama, bare fortsett.

---

## Uke 1 – Kartlegging + Funksjoner (15.–21. mars)
### Level 0 → starter på Level 1

> Hei Espen! Uke 1 starter med å finne ut nøyaktig hvor du står. Ikke vær redd for
> å gjøre det dårlig på det første eksamenssettet – det er *poenget*. Du kartlegger
> terrenget. Det verste du kan gjøre er å hoppe over dette og gå rett på teori.
> Fakta: alle som starter R2 fra bar bakke og jobber jevnt, klarer eksamen. Du er i gang.

- [ ] **Søn 15. mars** – Gjennomfør eksamensett 2024 vår under tidsbetingelser (Del 1: 2t, Del 2: 3t). Ikke slå opp. Fyll inn `eksamenssett/2024-vaar.md`. Oppdater `kartlegging.md`.
- [ ] **Man 16. mars** – Gå gjennom fasit 2024 vår grundig. Forstå *hvorfor* hvert svar er riktig. Ikke bare se hva du fikk feil – forstå metoden.
- [ ] **Tir 17. mars** – Polynomfunksjoner: nullpunkter, faktorisering, polynomdivisjon. Sinus R2 kap. 1 + øvingsoppgaver.
- [ ] **Ons 18. mars** – Rasjonale funksjoner: asymptoter, definisjonsområde, skisser. Øvingsoppgaver.
- [ ] **Tor 19. mars** – Eksponential- og logaritmefunksjoner. Regler, løse likninger. *Husk: øret ditt hører logaritmisk – desibel-skalaen er ln(x). Naturen bruker dette overalt.*
- [ ] **Fre 20. mars** – Plukk ut funksjonsoppgaver fra 2–3 tidligere eksamenssett. Gjør dem. Forklar fremgangsmåten høyt.
- [ ] **Lør 21. mars** – Gjennomfør eksamensett 2023 høst Del 1 (uten hjelpemidler, 2 timer). Rett og noter.

---

## Uke 2 – Funksjoner ferdig + Derivasjon starter (22.–28. mars)

> Du har nå gjennomført to eksamensett (eller deler av dem). Kartleggingen begynner
> å gi mening. Derivasjon er det viktigste emnet i R2 – det dukker opp overalt og
> brukes i nesten alle andre emner. Gi det tid. Det lønner seg.

- [ ] **Søn 22. mars** – Gjennomfør eksamensett 2023 høst Del 2 (med hjelpemidler, 3 timer). Oppdater kartlegging.
- [ ] **Man 23. mars** – Derivasjonsregler: potens, sum, konstant. Sinus R2 kap. 2. Gjør 10 oppgaver.
- [ ] **Tir 24. mars** – Produktregelen og kvotientregelen. Øvingsoppgaver. Disse to reglene er eksamensfavoritter.
- [ ] **Ons 25. mars** – Kjerneregelen. Sammensatte funksjoner. *Dette er den viktigste derivasjonsregelen. Bruk tid her.*
- [ ] **Tor 26. mars** – Monotoni og ekstremalpunkter. Fortegnsskjema for f'(x). Tegn grafer fra bunnen av.
- [ ] **Fre 27. mars** – Andrederiverte, vendepunkter, krumning.
- [ ] **Lør 28. mars** – Eksamensoppgaver derivasjon fra tidligere sett. Oppdater kartlegging.

> **Level 1 unlocked** når du klarer å løse en funksjonsoppgave fra Del 1 uten å
> måtte slå opp metoden. Oppdater `kartlegging.md`.

---

## Uke 3 – Derivasjon avansert + optimering (29. mars–4. april)
### Level 1 → Level 2

> Optimeringsproblemer er gjerne de tøffeste oppgavene på Del 1. De krever at du
> *selv* setter opp en funksjon fra en tekstoppgave og deriverer. Øv på å lese
> oppgaven nøye og identifisere hva som skal maksimeres eller minimeres. Vandrefalken
> som dykker på 320 km/h – det øyeblikket den er på topphastighet, er v'(t) = 0.
> Du finner det punktet med derivasjon. Det er akkurat det disse oppgavene handler om.

- [ ] **Søn 29. mars** – Gjennomfør eksamensett 2023 vår Del 1. Rett og noter.
- [ ] **Man 30. mars** – Optimeringsproblemer: sett opp funksjon fra tekstoppgave, deriver, finn ekstremum. Sinus R2 kap. 2.
- [ ] **Tir 31. mars** – Optimering del 2 – geometri: areal, volum, innpakningsproblemer. Gjør minst 5 oppgaver.
- [ ] **Ons 1. april** – Derivasjon av eksponential og logaritme: (e^x)' = e^x, (ln x)' = 1/x. Disse er eksamenssikre.
- [ ] **Tor 2. april** – Gjennomfør eksamensett 2023 vår Del 2. Oppdater kartlegging.
- [ ] **Fre 3. april** – Fasit 2023 vår. Fokus på Del 2-teknikk: bruk GeoGebra CAS til å sjekke svar.
- [ ] **Lør 4. april** – Repetisjon derivasjon: 10 oppgaver fra ulike sett. Forklar hvert steg skriftlig – tvinger deg til å forstå, ikke bare gjøre.

> **Level 2 unlocked** når du klarer et optimeringsproblem fra Del 1 uten hjelp.
> Det er et av de vanskeligste oppgavetypene i R2. Kan du det, er du godt forberedt.

---

## Uke 4 – Integrasjon (5.–11. april)
### Level 2 → Level 3

> Integrasjon er derivasjonens motsetning. Der derivasjon spør "hvor fort skjer dette
> nå?", spør integrasjon "hvor mye har skjedd totalt?" Klimaforskere beregner total
> CO₂-utslipp ved å integrere utslippsraten over 100 år. Når en elv flommer over,
> er det totale vannvolumet integralet av vannføringen over tid.
> Dette er ikke abstrakt matte – dette er verktøyene verden styres med.

- [ ] **Søn 5. april** – Gjennomfør eksamensett 2022 høst Del 1. Rett og noter.
- [ ] **Man 6. april** – Ubestemte integraler: potensregelen, sum, konstant. Sinus R2 kap. 3. Drill grunnreglene.
- [ ] **Tir 7. april** – Bestemte integraler: arealtolkning, regning. Forstå sammenhengen mellom integral og areal.
- [ ] **Ons 8. april** – Areal mellom to kurver. Finn skjæringspunkter, sett opp integralet riktig. Vanlig eksamensfeil: feil fortegn.
- [ ] **Tor 9. april** – Volum ved rotasjon rundt x-aksen (Disk-metoden). *Hvis du roterer en kurve rundt en akse, kan du beregne volumet av den tredimensjonale figuren – skikkelig kult.*
- [ ] **Fre 10. april** – Integrasjon ved substitusjon. Den vanskeligste integrasjonsteknikken – gi den tid.
- [ ] **Lør 11. april** – Gjennomfør eksamensett 2022 høst Del 2. Oppdater kartlegging.

---

## Uke 5 – Integrasjon ferdig + Differensiallikninger (12.–18. april)
### Level 3 → Level 4

> Differensiallikninger er kanskje det morsomste emnet i R2 – fordi de beskriver
> *alt som endrer seg*. Kullstoff-14-datering: dy/dt = −ky. Populasjonsvekst: samme
> form. Newtons avkjølingslov (hvordan en kaffekjele kjøler seg ned): også samme form.
> Politiet bruker dette til å estimere dødstidspunktet i drapssaker.
> Forstår du én differensiallikning, forstår du mange fenomener i naturen på én gang.

- [ ] **Søn 12. april** – Gå gjennom fasit 2022 høst. Fokus på integrasjonsoppgaver du ikke fikk til.
- [ ] **Man 13. april** – Delbrøkoppspalting. Krevende, men dukker opp. Gjør noen oppgaver fra Sinus R2.
- [ ] **Tir 14. april** – Separable differensiallikninger: skill variabler, integrer begge sider. Øv på å kjenne igjen typen.
- [ ] **Ons 15. april** – Lineære 1. ordens differensiallikninger (integrerende faktor). Lær metoden steg for steg.
- [ ] **Tor 16. april** – Eksamensoppgaver differensiallikninger fra tidligere sett. Minst 5 oppgaver.
- [ ] **Fre 17. april** – Gjennomfør eksamensett 2022 vår Del 1. Rett og noter.
- [ ] **Lør 18. april** – Gjennomfør eksamensett 2022 vår Del 2. Oppdater kartlegging.

> **Level 3 unlocked** når du klarer et integraloppgave fra Del 1 uten å nøle.
> **Level 4 unlocked** når du kan løse en separabel differensiallikning fra bunnen.

---

## Uke 6 – Rekker, følger + Vektorer starter (19.–25. april)

> Tar du én tablett med 500 mg paracetamol, og 50% forlater kroppen din per time,
> er mengden etter n timer en geometrisk følge: 500, 250, 125, 62.5 ...
> Den totale mengden absorberte stoffet er summen av en uendelig geometrisk rekke –
> og den konvergerer til et endelig tall. Kroppen din er matematikk.
> Uke 6 introduserer også vektorer – navigasjonens språk.

- [ ] **Søn 19. april** – Aritmetiske følger og rekker: formel for ledd og sum. Sinus R2 kap. 5.
- [ ] **Man 20. april** – Geometriske følger og rekker: formel, konvergens, uendelig sum. *Øv på å avgjøre om en rekke konvergerer.*
- [ ] **Tir 21. april** – Eksamensoppgaver rekker/følger fra tidligere sett. Vanlig oppgavetype: innskudd i bank over tid.
- [ ] **Ons 22. april** – Vektorer i 2D og 3D: addisjon, skalering, lengde. Sinus R2 kap. 6. *GPS bruker vektorer – telefonen din løser dette i sanntid.*
- [ ] **Tor 23. april** – Skalarprodukt, vinkel mellom vektorer, ortogonalitet.
- [ ] **Fre 24. april** – Kryssprodukt, areal av parallellogram, volum av parallelepiped.
- [ ] **Lør 25. april** – Gjennomfør eksamensett 2021 høst Del 1. Rett og noter.

---

## Uke 7 – Vektorer ferdig + Sannsynlighet (26. april–2. mai)
### Level 4 → Level 5 → **Level 6: PENSUM FERDIG**

> Dette er den siste pensumuken, Espen. Etter lørdag 2. mai har du gått gjennom
> hele R2-pensum. Det er ikke mange som kan si det. Vektorer i 3D er elegant –
> det handler om å beskrive linjer og plan i rommet, og er grunnlaget for 3D-grafikk,
> arkitektur og romfart. Sannsynlighet og normalfordelingen avslutter pensum:
> laksens lengde i en norsk elv, blodtrykk hos voksne, IQ-score i en befolkning –
> alt følger normalfordelingen. Det er universets favorittform.

- [ ] **Søn 26. april** – Gjennomfør eksamensett 2021 høst Del 2. Oppdater kartlegging.
- [ ] **Man 27. april** – Linjer på parameterform. Finn skjæringspunkt mellom linjer, avstand fra punkt til linje.
- [ ] **Tir 28. april** – Plan på normalform. Avstand fra punkt til plan. *Arkitekter og spillutviklere bruker dette daglig.*
- [ ] **Ons 29. april** – Kombinatorikk: permutasjoner, kombinasjoner, binomialkoeffisienter. Sinus R2 kap. 7.
- [ ] **Tor 30. april** – Binomialfordeling: P(X=k), forventningsverdi, standardavvik. Typisk oppgave: myntkast, skudd mot mål.
- [ ] **Fre 1. mai** – Normalfordeling: standardisering, z-verdi, bruk av tabell og GeoGebra. *Forstår du normalfordelingen, forstår du statistikk.*
- [ ] **Lør 2. mai** – Eksamensoppgaver sannsynlighet fra tidligere sett. Oppdater kartlegging.

> **Level 5 + Level 6 unlocked. Hele pensum er dekket.**
> Fra nå av handler det ikke om å lære nytt – det handler om å bli trygg.

---

## Uke 8 – Eksamensett + svake emner (3.–9. mai)
### Boss Prep, fase 1

> Alt pensum er i boks. Nå er jobben å finne hullene og tette dem. Ikke prøv å
> repetere alt – bruk kartlegging.md og jobb målrettet med det som er rødt.
> Hvert eksamenssett du gjennomfører nå bygger selvtillit og teknikk.
> Du er nærmere enn du tror, Espen.

- [ ] **Søn 3. mai** – Gjennomfør eksamensett 2021 vår som simulert eksamen (Del 1 + Del 2 sammenhengende, under tidsbetingelser). Rett grundig. Oppdater `kartlegging.md`.
- [ ] **Man 4. mai** – Gå i dybden på svakeste emne (se 🔴 i kartlegging.md). Sinus R2 + eksamensoppgaver målrettet.
- [ ] **Tir 5. mai** – Gjennomfør eksamensett 2020 vår Del 1 (2 timer, ingen hjelpemidler). Rett, tidsanalyse.
- [ ] **Ons 6. mai** – Gjennomfør eksamensett 2020 vår Del 2 (3 timer). Rett, oppdater kartlegging.
- [ ] **Tor 7. mai** – Fordyp deg i nest svakeste emne. Eksamensoppgaver kun på det emnet.
- [ ] **Fre 8. mai** – Gjennomfør eksamensett 2019 vår Del 1. Fokus på tempo – kan du fullføre i tid?
- [ ] **Lør 9. mai** – Gjennomfør eksamensett 2019 vår Del 2. Rett og oppdater kartlegging.

---

## Uke 9 – Eksamensett + siste finpuss (10.–14. mai)
### Boss Prep, fase 2

> Siste uke, Espen. Ingen nytt stoff. Nå handler det om å konsolidere, bli trygg
> på teknikken og gå inn til eksamen med selvtillit. Du har lagt ned jobben.
> Husk: du trenger ikke perfekte svar – du trenger å vise at du forstår metoden.
> Delpoeng for riktig fremgangsmåte, selv om svaret er galt.

- [ ] **Søn 10. mai** – Gjennomfør eksamensett 2022 vår som siste fulle simulering. Rett grundig.
- [ ] **Man 11. mai** – Gå gjennom alle emner med 🔴/🟡 i kartlegging.md. Gjør 2–3 oppgaver per emne.
- [ ] **Tir 12. mai** – Del 1-sprint: 15 oppgaver tidsbestemt (maks 8 min per oppgave). Forklar hvert svar.
- [ ] **Ons 13. mai** – Gå gjennom egne feiloppgaver fra hele perioden. Lag eget formelark. Ikke nytt stoff.
- [ ] **Tor 14. mai** – Gjør deg klar: pakk kalkulator, blyant, linjal. Les gjennom formelarket. Spis godt. Legg deg tidlig. Du er klar.

---

## 15. mai – EKSAMEN

> Du har jobbet for dette i 9 uker. Nå er det bare å vise hva du kan.
>
> **Del 1:** Ingen kalkulator – les oppgavene nøye, vis alle utregninger, ikke hopp over steg.
> Ufullstendige svar gir delpoeng. Sitter du fast: gå videre, kom tilbake.
>
> **Del 2:** Bruk GeoGebra aktivt. Sjekk svar grafisk. Vis alltid utregning – sensor
> gir ikke poeng for bare et tall. Les oppgaven to ganger. R2-oppgaver har ofte
> skjulte betingelser som endrer alt.
>
> Lykke til, Espen. Du fortjener det.

---

## Eksamensett å bruke (nyeste først)

| Sett | Status | Fil |
|------|--------|-----|
| 2024 vår | | eksamenssett/2024-vaar.md |
| 2023 høst | | eksamenssett/2023-hoest.md |
| 2023 vår | | eksamenssett/2023-vaar.md |
| 2022 høst | | eksamenssett/2022-hoest.md |
| 2022 vår | | eksamenssett/2022-vaar.md |
| 2021 høst | | eksamenssett/2021-hoest.md |
| 2021 vår | | eksamenssett/2021-vaar.md |
| 2020 vår | | eksamenssett/2020-vaar.md |
| 2019 vår | | eksamenssett/2019-vaar.md |

Eksamensoppgaver + løsningsforslag: https://ndla.no/r/matematikk-r2/eksamensoppgaver-og-losninger-i-r2/a9b4413421
