# R2 Matematikk – Eksamensplan

**Eksamensdato:** 15. mai 2026
**Startpunkt:** Relativt bar bakke
**Tilgjengelig tid:** ~9 uker (fra 15. mars)

---

## Pensum R2 (Matematikk 2 – Realfag)

R2-eksamen tester:

- **Funksjoner og algebra** – polynomfunksjoner, rasjonale funksjoner, logaritme/eksponentialfunksjoner
- **Derivasjon** – regler, optimering, andrederiverte og krumning
- **Integrasjon** – ubestemte og bestemte integraler, flate under/mellom kurver, volum
- **Differensiallikninger** – separable og lineære 1. ordens
- **Rekker og følger** – aritmetiske/geometriske, summer
- **Vektorer i 2D og 3D** – skalarprodukt, kryssprodukt, linjer og plan
- **Sannsynlighet og statistikk** – kombinatorikk, binomialfordeling, normalfordeling

---

## Ukeplan (9 uker)

### Uke 1–2 (15.–29. mars): Funksjoner og algebra
- [ ] Polynomfunksjoner, rasjonale funksjoner
- [ ] Eksponential- og logaritmefunksjoner
- [ ] Løse likninger og ulikheter
- [ ] Øvingsoppgaver fra tidligere eksamener
- **Ressurs:** Kikora / NDLA matematikk R2

### Uke 3–4 (30. mars–12. april): Derivasjon
- [ ] Derivasjonsregler (produkt, kvotient, kjerneregel)
- [ ] Monotoni, ekstremalpunkter, vendepunkter
- [ ] Optimeringsoppgaver
- [ ] Andrederiverte og krumning
- **Ressurs:** Eksamensoppgaver 2019–2024 del 1

### Uke 5 (13.–19. april): Integrasjon
- [ ] Ubestemte integraler og integrasjonsregler
- [ ] Bestemte integraler
- [ ] Areal mellom kurver
- [ ] Volum ved rotasjon
- [ ] Integrasjon ved substitusjon og delbrøkoppspalting

### Uke 6 (20.–26. april): Differensiallikninger + Rekker/Følger
- [ ] Separable differensiallikninger
- [ ] Lineære 1. ordens differensiallikninger
- [ ] Aritmetiske og geometriske følger og rekker
- [ ] Konvergens/divergens

### Uke 7 (27. april–3. mai): Vektorer
- [ ] Vektorer i 2D og 3D
- [ ] Skalarprodukt og vinkel mellom vektorer
- [ ] Kryssprodukt og areal/volum
- [ ] Linjer og plan på parameterform og normalform

### Uke 8 (4.–10. mai): Sannsynlighet + Del 2-trening
- [ ] Kombinatorikk (permutasjoner, kombinasjoner)
- [ ] Binomialfordeling
- [ ] Normalfordeling og standardisering
- [ ] Gjennomgang av typiske Del 2-oppgaver (CAS/GeoGebra)

### Uke 9 (11.–14. mai): Eksamensforberedelse
- [ ] Gjennomgang av egne svakheter
- [ ] Simuler eksamen under tidsbetingelser
- [ ] Repetisjon av formler og standardprosedyrer
- [ ] Hvile og forberedelse dagen før

---

## Hjelpemidler på eksamen

**Del 1 (uten hjelpemidler):** Kun formelark
**Del 2 (med hjelpemidler):** Kalkulator (Casio fx-9860G eller tilsvarende), GeoGebra, alle hjelpemidler unntatt kommunikasjon

---

## Ressurser

| Ressurs | Lenke |
|--------|-------|
| NDLA R2 | https://ndla.no/subject:1:3b9ff18e-f4c8-4d55-8823-97879aeac1e6 |
| Matematikk.org | https://matematikk.org |
| Eksamensoppgaver | https://matematikk.org/side/Eksamensoppgaver_R2 |
| Kikora | https://kikora.no |
| GeoGebra | https://geogebra.org |

---

## Kartlegging og eksamenssett

- **[kartlegging.md](kartlegging.md)** – statusoversikt over alle emner (🔴/🟡/🟢), oppdateres etter hvert sett
- **[eksamenssett/mal.md](eksamenssett/mal.md)** – mal for logging av hvert sett
- Kopier malen til `eksamenssett/YYYY-vaar.md` for hvert sett som gjennomgås

**Anbefalt arbeidsflyt:**
1. Gjennomfør et eksamensett under reelle betingelser (tidsbruk, uten å slå opp)
2. Rett og fyll inn poeng i sett-loggen
3. Oppdater `kartlegging.md` med nye observasjoner
4. Juster prioriteringslisten og ukeplanen deretter

---

## Fremgang

Oppdater checkbokser løpende. Eksamensresultat etter 15. mai.

---

## Tips

- Prioriter Del 1-ferdigheter (uten kalkulator) — her skiller de flinke seg ut
- GeoGebra er kraftig til Del 2 – øv spesifikt på CAS og 3D-grafikk
- Les oppgaver nøye — R2 krever presise svar med begrunnelse
- Gjennomgå sensurveildning for tidligere eksamener for å forstå hva sensor vil se
